...
void 
helpers::Encode_S(std::unordered_map<std::string, memory::Register> &regs, int &PC, std::string &binary_string, std::string &raw_string, std::vector<memory::byte> &data__stack_mem, int &data_ptr, int &stack_ptr)
{
    auto s3 = [](int i) -> std::string
    {
        return std::bitset<3>(i).to_string();
